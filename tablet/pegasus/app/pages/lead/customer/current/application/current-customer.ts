...
export class CurrentCustomerPage implements OnInit {
        ...
        if(this.appProvider.current.lead.cif==null) {
             // alert(this.appProvider.current.lead.cif)
                    ...
                    .subscribe(data => {
                            ...
                            console.log(this.customers['cif'])
                            localStorage['cif']=this.customers['cif'];
                            console.log(this.customers['message'])
                            // alert(JSON.stringify(this.account))
                            // alert(JSON.stringify(this.account))
                            // alert('1')
                            // alert(this.account[0].accountStatus)
                            // alert('2')
                            // alert(this.account.accountStatus)
    ...
    onNext(){
             this.navController.push(CurrentApplicationPage,{accountsumm:this.accountsumm,lead:this.currentAccount}); 
       //   let loading = this.loadingCtrl.create({content: this.translateService.instant('action.processing')});
       //  Observable
       //      .fromPromise(loading.present())
       //      .flatMap(() => this.cordovaProvider.checkMainApi())
