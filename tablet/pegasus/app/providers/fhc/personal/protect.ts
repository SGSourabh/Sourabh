...
import {Protect, ProtectBuilder, ApiCreateProtectResponseData} from "../../../models/fhc/personal/protect";
import {ENV} from "../../../env";
...
export class ProtectProvider {
    ...
    readProtect(id: number): Observable<Protect> {
        this.logProvider.info('provider', 'personal', 'protect', 'readProtect');
        ...
        return this.http.get(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/protects/' + id, options)
            .timeout(ENV.timeout)
            .map(response => {
    ...
    createProtect(protect: Protect): Observable<Protect> {
        this.logProvider.info('provider', 'personal', 'protect', 'createProtect');
        ...
        return this.http.post(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/protects', body, options)
            .timeout(ENV.timeout)
            .map(response => {
    ...
    updateProtect(protect: Protect): Observable<Protect> {
        this.logProvider.info('provider', 'personal', 'protect', 'updateProtect');
        ...
        return this.http.patch(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/protects/' + protect.id, body, options)
            .timeout(ENV.timeout)
            .map(response => {
                if (response.status !== 200) {
                    throw Error(this.translateService.instant('message.failed'));
                ...
                let apiCreateProtectResponseData: ApiCreateProtectResponseData = response.json();
                return new ProtectBuilder()
                    .fromApiCreateProtectResponseData(apiCreateProtectResponseData).setId(protect.id)
                    .setLeadId(this.appProvider.current.lead.id)
                    .build();
            })
            .catch(error => {
