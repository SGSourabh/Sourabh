...
import {Lending, LendingBuilder, ApiCreateLendingResponseData} from "../../../models/fhc/personal/lending";
import {ENV} from "../../../env";
...
export class LendingProvider {
    ...
    readLending(id: number): Observable<Lending> {
        this.logProvider.info('provider', 'personal', 'lending', 'readLending');
        ...
        return this.http.get(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/lendings/' + id, options)
            .timeout(ENV.timeout)
            .map(response => {
    ...
    createLending(lending: Lending): Observable<Lending> {
        this.logProvider.info('provider', 'personal', 'lending', 'createLending');
        ...
        return this.http.post(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/lendings', body, options)
            .timeout(ENV.timeout)
            .map(response => {
    ...
    updateLending(lending: Lending): Observable<Lending> {
        this.logProvider.info('provider', 'personal', 'lending', 'updateLending');
        ...
        return this.http.patch(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/lendings/' + lending.id, body, options)
            .timeout(ENV.timeout)
            .map(response => {
                if (response.status !== 200) {
                    throw Error(this.translateService.instant('message.failed'));
                ...
                return new LendingBuilder()
                    .fromApiCreateLendingResponseData(apiCreateLendingResponseData)
                    .setId(lending.id).setLeadId(this.appProvider.current.lead.id)
                    .build();
            })
            .catch(error => {
                if (error.status === 401) {
