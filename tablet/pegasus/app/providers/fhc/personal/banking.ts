...
import {Banking, BankingBuilder, ApiCreateBankingResponseData} from "../../../models/fhc/personal/banking";
import {ENV} from "../../../env";
...
export class BankingProvider {
    ...
    readBanking(id: number): Observable<Banking> {
        this.logProvider.info('provider', 'personal', 'banking', 'readBanking');
        ...
        return this.http.get(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/bankings/' + id, options)
            .timeout(ENV.timeout)
            .map(response => {
    ...
    createBanking(banking: Banking): Observable<Banking> {
        this.logProvider.info('provider', 'personal', 'banking', 'createBanking');
        ...
        return this.http.post(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/bankings', body, options)
            .timeout(ENV.timeout)
            .map(response => {
    ...
    updateBanking(banking: Banking): Observable<Banking> {
        this.logProvider.info('provider', 'personal', 'banking', 'updateBanking');
        ...
        return this.http.patch(ENV.mainApi + '/leads/' + this.appProvider.current.lead.id + '/fhcs/personals/bankings/' + banking.id, body, options)
            .timeout(ENV.timeout)
            .map(response => {
                if (response.status !== 200) {
                    throw Error(this.translateService.instant('message.failed'));
