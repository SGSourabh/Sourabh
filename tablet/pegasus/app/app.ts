...
class MyApp implements OnInit {
    ...
    ngOnInit() {
        ...
        this.translateService.use(ENV.language).flatMap(() => this.cordovaProvider.ready()).subscribe(() => {
            ...
            this.idle.watch();
            this.cordovaProvider.startTrackerWithId();
            ...
            if (ENV.bypass) {
                localStorage['user_id'] = 'vib_user';
                return this.nav.setRoot(LeadPage);
            }
            this.securityProvider.registered()
                .subscribe(registered => {
                        this.userProvider.handle = registered.handle;
                        this.nav.setRoot(LoginPage);
                    },
                    error =>
                        error === 'timeout' ? this.appProvider.createAlert(error, () => this.ngOnInit()).present() : this.nav.setRoot(RegistrationPage)
                );
        }, error => {
            if (ENV.bypass) {
                return this.nav.setRoot(LeadPage);
            ...
            this.alertController
                .create({subTitle: error, buttons: [this.translateService.instant('action.close')]})
                .present();
        });
    }
