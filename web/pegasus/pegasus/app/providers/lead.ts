...
export class LeadProvider {
    ...
    readLeads(leadSearch: LeadSearch): Observable<Lead[]> {
        ...
        if (leadSearch) {
            ...
            if (leadSearch.index>1) {
                params.set('index', leadSearch.index.toString());
            }
        }
    ...
    readCustomers(lead: Lead,index?:any): Observable<Customer[]> {
    ...
    readLeadsDashbordPre(from:string,to:string,sort:any): Observable<Lead[]> {
     ...
     readLeadsCurrent(from:string,to:string,sort:any): Observable<Lead[]> {
        this.logProvider.info('provider', 'leads', 'readLeadsDashbord');
        let params: URLSearchParams = new URLSearchParams();
        let extraPath: string ="";
        params.set('owner', this.userProvider.user.name);
